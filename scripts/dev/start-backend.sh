...
if [[ -n "$existing_pid" ]]; then
  if [[ "${REPLACE_PORT:-0}" == "1" ]]; then
    ...
    echo "Run REPLACE_PORT=1 scripts/dev/start-backend.sh, or scripts/dev/stop-backend.sh --force-port first." >&2
    exit 1
  fi
fi
...
CMD=(
  ...
  --spring.sql.init.schema-locations="file:$ROOT_DIR/lineage-server/lineage-server-start/src/main/resources/scripts/h2/schema.sql"
  --spring.sql.init.data-locations="file:$ROOT_DIR/lineage-server/lineage-server-start/src/main/resources/scripts/h2/data.sql"
